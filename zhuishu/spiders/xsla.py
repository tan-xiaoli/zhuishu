...
from zhuishu.lib.lib_tools import get_md5, cn2dig


class XsLaSpider(Spider):
    """
    笔趣阁http://www.xs.la/
    """
    ...
    name = 'xs.la'          # 爬虫名
    allowed_domains = ['xs.la']  # 限定域名

    # 起始URL,目录页
    start_urls = [
        'http://xs.la/3_3253/',  # 帝霸
        'http://xs.la/0_4/',  # 大主宰
        'http://xs.la/59_59878/',   # 龙王传说
    ]
    ...
    def parse(self, response):
        ...
        book['chapter_db_name'] = get_md5(book['book_name'])
        # book['chapter_db_name'] = book['book_name']

        if not XsLaSpider.spider_all_chapter:
    ...
    def parse_content(self, response):
        ...
        content_path = os.path.join(settings.BOOK_SAVE_PATH, '%s/%s/%s_%s.txt' %
                                    (XsLaSpider.name, get_md5(chp['book_name']), chp['chapter'],
                                        get_md5(chp['title'])))
